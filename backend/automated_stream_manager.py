...
try:
    ...
    logger.warning(
        "DeadStreamsTracker not available. Dead stream filtering will be disabled.")
    ...
    def _load_changelog(self) -> List[Dict]:
        ...
        if self.changelog_file.exists():
            try:
                ...
                logger.warning(
                    f"Could not load {self.changelog_file}, creating new changelog")
        return []
...
class RegexChannelMatcher:
    ...
    def _load_patterns(self) -> Dict:
        ...
        if self.config_file.exists():
            try:
                ...
                logger.warning(
                    f"Could not load {self.config_file}, creating default config")
    ...
    def validate_regex_patterns(
            self, patterns: List[str]) -> Tuple[bool, Optional[str]]:
    ...
    def match_stream_to_channels(self, stream_name: str) -> List[str]:
        ...
        matches = []
        case_sensitive = self.channel_patterns.get(
            "global_settings", {}).get(
            "case_sensitive", False)

        search_name = stream_name if case_sensitive else stream_name.lower()

        for channel_id, config in self.channel_patterns.get(
                "patterns", {}).items():
            ...
            for pattern in config.get("regex", []):
                ...
                try:
                    if re.search(search_pattern, search_name):
                        ...
                        logger.debug(
                            f"Stream '{stream_name}' matched channel {channel_id} with pattern '{pattern}'")
                        break  # Only match once per channel
                except re.error as e:
                    logger.error(
                        f"Invalid regex pattern '{pattern}' for channel {channel_id}: {e}")
...
class AutomatedStreamManager:
    ...
    def __init__(self, config_file=None):
        ...
        if DEAD_STREAMS_TRACKER_AVAILABLE:
            try:
                ...
                logger.warning(
                    f"Failed to initialize dead streams tracker: {e}")
    ...
    def update_config(self, updates: Dict):
        ...
        if 'playlist_update_interval_minutes' in updates:
            old_interval = self.config.get(
                'playlist_update_interval_minutes', 5)
            new_interval = updates['playlist_update_interval_minutes']
            if old_interval != new_interval:
                config_changes.append(
                    f"Playlist update interval: {old_interval} → {new_interval} minutes")
        ...
        if 'enabled_m3u_accounts' in updates:
            ...
            if old_accounts != new_accounts:
                if not new_accounts:
                    ...
                    config_changes.append(
                        f"M3U accounts: {len(new_accounts)} selected")
        ...
        if config_changes:
            logger.info(
                f"Automation configuration updated: {'; '.join(config_changes)}")
    ...
    def refresh_playlists(self, force: bool = False) -> bool:
        ...
        with cache:
            try:
                if not force and not self.config.get(
                        "enabled_features", {}).get(
                        "auto_playlist_update", True):
                    logger.info("Playlist update is disabled in configuration")
                    return False

                logger.info(
                    "Starting M3U playlist refresh (with caching enabled)...")

                # Get streams before refresh
                from api_utils import get_streams
                streams_before = get_streams(
                    log_result=False) if self.config.get(
                    "enabled_features",
                    {}).get(
                    "changelog_tracking",
                    True) else []
                before_stream_ids = {
                    s.get('id'): s.get(
                        'name', '') for s in streams_before if isinstance(
                        s, dict) and s.get('id')}

                # Get all M3U accounts and filter out "custom" and non-active
                # accounts
                ...
                if all_accounts:
                    ...
                    # Only filter by name, not by null URLs, as legitimate
                    # accounts may have these
                    non_custom_accounts = [
                        acc for acc in all_accounts if acc.get(
                            'name', '').lower() != 'custom' and acc.get(
                            'is_active', True)]

                    # Perform refresh - check if we need to filter by enabled
                    # accounts
                    enabled_accounts = self.config.get(
                        "enabled_m3u_accounts", [])
                    if enabled_accounts:
                        # Refresh only enabled accounts (and exclude custom)
                        non_custom_ids = [
                            acc.get('id') for acc in non_custom_accounts if acc.get('id') is not None]
                        accounts_to_refresh = [
                            acc_id for acc_id in enabled_accounts if acc_id in non_custom_ids]
                        ...
                        if len(enabled_accounts) != len(accounts_to_refresh):
                            logger.info(
                                f"Skipped {len(enabled_accounts) - len(accounts_to_refresh)} account(s) (custom or invalid)")
                    else:
                        ...
                        for account in non_custom_accounts:
                            ...
                            if account_id is not None:
                                logger.info(
                                    f"Refreshing M3U account {account_id}")
                                refresh_m3u_playlists(account_id=account_id)
                        if len(all_accounts) != len(non_custom_accounts):
                            logger.info(
                                f"Skipped {len(all_accounts) - len(non_custom_accounts)} 'custom' account(s)")
                else:
                    # Fallback: if we can't get accounts, refresh all (legacy
                    # behavior)
                    logger.warning(
                ...
                # final result
                streams_after = get_streams(
                    log_result=True) if self.config.get(
                    "enabled_features",
                    {}).get(
                    "changelog_tracking",
                    True) else []
                after_stream_ids = {
                    s.get('id'): s.get(
                        'name', '') for s in streams_after if isinstance(
                        s, dict) and s.get('id')}

                self.last_playlist_update = datetime.now()

                # Calculate differences
                added_stream_ids = set(
                    after_stream_ids.keys()) - set(before_stream_ids.keys())
                removed_stream_ids = set(
                    before_stream_ids.keys()) - set(after_stream_ids.keys())

                added_streams = [{"id": sid, "name": after_stream_ids[sid]}
                                 for sid in added_stream_ids]
                removed_streams = [
                    {"id": sid, "name": before_stream_ids[sid]} for sid in removed_stream_ids]
                ...
                if self.config.get(
                    ...
                    self.changelog.add_entry("playlist_refresh", {
                        ...
                        # Limit to first 50 for changelog size
                ...
                logger.info(
                    f"M3U playlist refresh completed successfully. Added: {len(added_streams)}, Removed: {len(removed_streams)}")
                ...
                if self.dead_streams_tracker:
                    try:
                        current_stream_urls = {
                            s.get(
                                'url',
                                '') for s in streams_after if isinstance(
                                s,
                                dict) and s.get('url')}
                        ...
                        if cleaned_count > 0:
                            logger.info(
                                f"Dead streams cleanup: removed {cleaned_count} stream(s) no longer in playlist")
                    except Exception as cleanup_error:
                        logger.error(
                            f"Error during dead streams cleanup: {cleanup_error}")

                # Mark channels for stream quality checking ONLY if streams were added or removed
                # This prevents unnecessary marking of all channels on every
                # refresh
                if len(added_streams) > 0 or len(removed_streams) > 0:
                    try:
                        ...
                        channels_data = fetch_data_from_url(
                            f"{base_url}/api/channels/channels/")

                        if channels_data:
                            if isinstance(
                                    channels_data,
                                    dict) and 'results' in channels_data:
                                channels = channels_data['results']
                            else:
                                channels = channels_data

                            # Mark all channels for checking with stream counts
                            # for 2-hour immunity
                            channel_ids = []
                            stream_counts = {}
                            for ch in channels:
                                if isinstance(ch, dict) and 'id' in ch:
                                    ...
                                    if 'streams' in ch and isinstance(
                                            ch['streams'], list):
                                        stream_counts[ch_id] = len(
                                            ch['streams'])

                            # Try to get stream checker service and mark
                            # channels
                            try:
                                ...
                                stream_checker.update_tracker.mark_channels_updated(
                                    channel_ids, stream_counts=stream_counts)
                                logger.info(
                                    f"Marked {len(channel_ids)} channels for stream quality checking")
                                # Trigger immediate check instead of waiting
                                # for scheduled interval
                                stream_checker.trigger_check_updated_channels()
                            except Exception as sc_error:
                                logger.debug(
                                    f"Stream checker not available or error marking channels: {sc_error}")
                    except Exception as ch_error:
                        logger.debug(
                            f"Could not mark channels for stream checking: {ch_error}")
                else:
                    logger.info(
                        "No stream changes detected, skipping channel marking")
    ...
    def discover_and_assign_streams(
            self, force: bool = False) -> Dict[str, int]:
        ...
        with cache:
            try:
                ...
                logger.info(
                    "Starting stream discovery and assignment (with caching enabled)...")

                # Get all available streams (don't log, we already logged
                # during refresh)
                ...
                if not isinstance(all_streams, list):
                    logger.error(
                        f"Invalid streams response format: expected list, got {type(all_streams).__name__}")
                    return {}
                ...
                if all_accounts:
                    ...
                    enabled_accounts_config = self.config.get(
                        "enabled_m3u_accounts", [])
                    ...
                    if enabled_accounts_config:
                        ...
                        # If no specific accounts are enabled in config, use
                    ...
                    # Also include custom streams (is_custom=True) as they
                    # don't belong to an M3U account
                    filtered_streams = [stream for stream in all_streams if stream.get(
                        'is_custom', False) or stream.get('m3u_account') in enabled_account_ids]

                    streams_filtered_count = len(
                        all_streams) - len(filtered_streams)
                    if streams_filtered_count > 0:
                        logger.info(
                            f"Filtered out {streams_filtered_count} streams from disabled/inactive M3U accounts")

                    all_streams = filtered_streams

                    if not all_streams:
                        logger.info(
                            "No streams found after filtering by enabled M3U accounts")
                        return {}
                else:
                    logger.warning(
                        "Could not fetch M3U accounts, using all streams")

                # Get all channels
                base_url = _get_base_url()
                all_channels = fetch_data_from_url(
                    f"{base_url}/api/channels/channels/")
                if not all_channels:
                ...
                if not isinstance(all_channels, list):
                    logger.error(
                        f"Invalid channels response format: expected list, got {type(all_channels).__name__}")
                    return {}
                ...
                for channel in all_channels:
                    ...
                    if not isinstance(channel, dict) or 'id' not in channel:
                        logger.warning(
                            f"Invalid channel format encountered: {type(channel).__name__} - {channel}")
                        continue

                    channel_id = str(channel['id'])
                    channel_names[channel_id] = channel.get(
                        'name', f'Channel {channel_id}')
                streams = fetch_data_from_url(
                    ...
                    if isinstance(streams, list):
                        ...
                        for s in streams:
                            if isinstance(s, dict) and 'id' in s:
                                ...
                                logger.warning(
                                    f"Invalid stream format in channel {channel_id}: {type(s).__name__} - {s}")
                        channel_streams[channel_id] = valid_stream_ids
                    else:
                        logger.warning(
                            f"Invalid streams format for channel {channel_id}: expected list, got {type(streams).__name__}")
                        channel_streams[channel_id] = set()
                else:
                    channel_streams[channel_id] = set()

                assignments = defaultdict(list)
                # Track stream details for changelog
                assignment_details = defaultdict(list)
                assignment_count = {}

                # Process each stream
                for stream in all_streams:
                    # Validate that stream is a dictionary before accessing
                    # attributes
                    if not isinstance(stream, dict):
                        logger.warning(
                            f"Invalid stream format encountered: {type(stream).__name__} - {stream}")
                        continue
                    ...
                    # Dead streams should not be added to channels during
                    # subsequent matches
                    stream_url = stream.get('url', '')
                    if self.dead_streams_tracker and self.dead_streams_tracker.is_dead(
                            stream_url):
                        logger.debug(
                            f"Skipping dead stream {stream_id}: {stream_name} (URL: {stream_url})")
                        continue
                    ...
                    for channel_id in matching_channels:
                        ...
                        if channel_id in channel_streams and stream_id not in channel_streams[
                                channel_id]:
                    ...
                    if stream_ids:
                        try:
                            added_count = add_streams_to_channel(
                                int(channel_id), stream_ids)
                            assignment_count[channel_id] = added_count

                            # Verify streams were added correctly
                            if added_count > 0:
                                try:
                                    # Brief delay for API processing
                                    time.sleep(0.5)
                                    base_url = _get_base_url()
                                    updated_streams = fetch_data_from_url(
                                        f"{base_url}/api/channels/channels/{channel_id}/streams/")

                                    if updated_streams and isinstance(
                                            updated_streams, list):
                                        updated_stream_ids = set(
                                            s.get('id') for s in updated_streams if isinstance(
                                                s, dict) and 'id' in s)
                                        expected_stream_ids = set(stream_ids)
                                        added_stream_ids = expected_stream_ids & updated_stream_ids

                                        if len(
                                                added_stream_ids) == added_count:
                                            logger.info(
                                                f"✓ Verified: {added_count} streams successfully added to channel {channel_id} ({channel_names.get(channel_id, f'Channel {channel_id}')})")
                                        else:
                                            logger.warning(
                                                f"⚠ Verification mismatch for channel {channel_id}: expected {added_count} streams, found {len(added_stream_ids)} in channel")
                                    else:
                                        logger.warning(
                                            f"⚠ Could not verify stream addition for channel {channel_id}: invalid response")
                                except Exception as verify_error:
                                    logger.warning(
                                        f"⚠ Could not verify stream addition for channel {channel_id}: {verify_error}")
                            ...
                            channel_assignment = {
                                ...
                                # Limit to first 20 for changelog
                                "streams": assignment_details[channel_id][:20]
                            }
                            detailed_assignments.append(channel_assignment)

                        except Exception as e:
                            logger.error(
                                f"Failed to assign streams to channel {channel_id}: {e}")
                ...
                if self.config.get(
                    ...
                    # Sort by stream count (descending) to show the most
                    # significant updates
                ...
                logger.info(
                    f"Stream discovery completed. Assigned {total_assigned} new streams across {len(assignment_count)} channels")

                # Mark channels that received new streams for stream quality
                # checking
                if total_assigned > 0 and assignment_count:
                    try:
                        # Get updated stream counts for channels that received
                        # new streams
                        channel_ids_to_mark = []
                        stream_counts = {}
                        ...
                        for channel_id in assignment_count.keys():
                            if assignment_count[channel_id] > 0:
                                ...
                                try:
                                    ch_streams = fetch_data_from_url(
                                        f"{base_url}/api/channels/channels/{channel_id}/streams/")
                                    if ch_streams and isinstance(
                                            ch_streams, list):
                                        stream_counts[int(channel_id)] = len(
                                            ch_streams)
                                except Exception:
                        ...
                        if channel_ids_to_mark:
                            try:
                                ...
                                stream_checker = get_stream_checker_service()
                                stream_checker.update_tracker.mark_channels_updated(
                                    channel_ids_to_mark, stream_counts=stream_counts)
                                logger.info(
                                    f"Marked {len(channel_ids_to_mark)} channels with new streams for stream quality checking")
                                # Trigger immediate check instead of waiting
                                # for scheduled interval
                                stream_checker.trigger_check_updated_channels()
                            except Exception as sc_error:
                        ...
                        logger.debug(
                            f"Could not mark channels for stream checking after discovery: {mark_error}")
    ...
    def run_automation_cycle(self):
        ...
        try:
            ...
            if stream_checker.global_action_in_progress:
                logger.debug(
                    "Skipping automation cycle - global action in progress")
                return
        except Exception as e:
    ...
    def start_automation(self):
        ...
        def automation_loop():
            ...
            while self.running:
                try:
                    ...
                    logger.debug(
                        "Automation cycle completed, sleeping for 60 seconds")
        ...
        self.automation_thread = threading.Thread(
            target=automation_loop, daemon=True)
        self.automation_thread.start()
        logger.debug(
            f"Automation thread started (id: {self.automation_thread.ident})")
    ...
    def get_status(self) -> Dict:
        ...
        if self.running:
            if self.last_playlist_update:
                # Calculate when the next update should occur based on last
                # update + interval
                next_update = self.last_playlist_update + \
                    timedelta(minutes=self.config.get("playlist_update_interval_minutes", 5))
            elif self.automation_start_time:
                # If automation is running but no last update, calculate from
                # start time
                next_update = self.automation_start_time + \
                    timedelta(minutes=self.config.get("playlist_update_interval_minutes", 5))
