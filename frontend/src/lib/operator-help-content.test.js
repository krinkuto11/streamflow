...
describe('operatorHelpSections', () => {
  ...
  it('provides platform-neutral detailed guide topics with settings explanations', () => {
    ...
    expect(streamChecker.smokeChecks.join(' ')).toMatch(/Capacity deferred.*provider\/profile slot/i)
    ...
    expect(needsRecheck.location).toBe('Stream Checker -> Current Progress -> Stream Progress Tracking -> Status')
