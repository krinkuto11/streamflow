export const operatorHelpSections = [
  ...
  },
  {
    ...
    items: [
      ...
      'Watched Now separates real viewers from Shadow probe clients; healthy channels may have no permanent watcher client between probes.',
      ...
      'Timing, cooldown, and switch-budget values are internal Continuous safeguards; normal operators should tune only Viewer Grace and watcher identity.',
    ],
    links: [
...
export const operatorHelpDetailTopics = [
  {
    ...
    settings: [
      ...
      },
      {
        ...
        location: 'Dashboard startup screen and /api/readiness',
        effect: 'Prevents automation and manual checks from using a partial database, configuration, runtime service, or Dispatcharr cache state.',
    ...
    smokeChecks: [
      '/api/health returns healthy process liveness.',
      '/api/readiness returns ready with the expected database, service, and Dispatcharr cache checks.',
  ...
  },
  {
    ...
    steps: [
      ...
      'When Bitrate Recheck appears, let the lightweight serial retry finish before treating the current bitrate as unavailable.',
    ...
    settings: [
      {
        ...
        risk: 'It still uses provider/profile and analysis capacity. Stream Checker, Automation, and Teamarr Preflight work must be idle; otherwise the API rejects the request with HTTP 409 instead of adding queue pressure.',
      },
      {
      ...
      },
      {
        ...
        controlType: 'Visible UI setting',
        defaultValue: '50',
        location: 'Stream Checker -> Stream Checker Configuration -> Edit -> Queue tab -> Max Channels Per Run',
      ...
      },
      {
        ...
        location: 'Stream Checker -> Stream Checker Configuration -> Edit -> Concurrent Checking tab -> Global Concurrent Limit',
        locationTo: '/stream-checker',
      ...
      },
      {
        ...
        risk: 'Do not use this fallback to model active profile capacity. Missing or malformed account/profile inventory fails closed, and if active profiles exist but none can resolve a usable route the check fails closed as provider_profile_unavailable instead of using the stored URL.',
      },
      {
        name: 'Provider profile limits',
        controlType: 'Status/API',
        defaultValue: 'Read from imported M3U account profiles',
        location: 'Stream Checker -> Current Progress (active run) -> Profile Matrix (expand); API GET /api/stream-checker/progress -> provider_progress[].profile_slots[]',
        locationTo: '/stream-checker',
        ...
        risk: 'Each probe URL must remain bound to the profile actually reserved. A default profile may use the stored URL; a non-default profile must produce its own valid credential rewrite or fail closed. Missing or malformed authority is never treated as an empty provider. The Profile Matrix is read-only status/API information, not an editable limits or credentials control.',
      },
      {
        name: 'Provider capacity authority',
        controlType: 'Status/API',
        defaultValue: 'Fail-closed when authority or live usage cannot be proved',
        location: 'Stream Checker -> Current Progress (active run) -> Stream Progress Tracking -> Status/reason; API GET /api/stream-checker/progress -> streams_detail[].reason_detail|skipped_reason',
        locationTo: '/stream-checker',
        effect: 'Distinguishes a genuinely full slot from provider_profile_unavailable (missing, malformed, or changed inventory/profile/route authority) and provider_usage_unavailable (live proxy status cannot prove current usage). Authority failures stop immediately; usage failures wait safely and may time out.',
        useWhen: 'Inspect this reason whenever a probe is deferred or skipped before changing provider limits or wait timeouts.',
        risk: 'Never treat either unavailable state as zero usage. Fix inventory, credential-route, or proxy-status health; raise capacity only when the reason proves genuine saturation.',
      },
      {
        name: 'Reserved probe profile',
        controlType: 'Status/API',
        defaultValue: 'Visible only for live stream progress that has used a profile',
        location: 'Stream Checker -> Current Progress (active run) -> Stream Progress Tracking -> Account (profile name; hover for ID and Limit); API GET /api/stream-checker/progress -> streams_detail[].reserved_profile_id|reserved_profile_name|reserved_profile_limit',
        locationTo: '/stream-checker',
        effect: 'Shows only the safe ID, name, and actually enforced limit of the profile used by the initial probe or serial bitrate recheck; shared credential aliases show their strict shared-route limit, and a recheck may replace profile A with profile B.',
        useWhen: 'Confirm the probe used the expected credential profile without exposing its URL or credentials.',
        risk: 'Read-only live telemetry; it clears on capacity wait or viewer preemption so a released profile is not advertised, and it is not retained as Changelog history.',
      },
      {
        name: 'CPU Fallback',
        controlType: 'Visible UI setting',
        defaultValue: 'On for safer hardware mode',
        location: 'Stream Checker -> Stream Checker Configuration -> Edit -> Stream Analysis tab -> Hardware Acceleration -> CPU Fallback',
        locationTo: '/stream-checker',
      ...
      {
        name: 'Bitrate Recheck',
        controlType: 'Status/API',
        defaultValue: 'Automatic when a playable probe has no current bitrate',
        location: 'Stream Checker -> Current Progress (active run) -> Stream Progress Tracking -> Status -> Bitrate Recheck',
        locationTo: '/stream-checker',
        effect: 'After the initial probes for a channel finish, StreamFlow retries only affected streams one at a time with a lightweight media probe before advancing to the next channel.',
        useWhen: 'Wait for this status on slower systems or busy providers where a successful initial probe reports no bitrate.',
        risk: 'This is not a user setting. If the serial retry still has no bitrate, the current result remains N/A and is labeled Bitrate unavailable after recheck; an older bitrate may influence ranking only and is not shown as the current measurement.',
      },
      {
        name: 'Bitrate Recheck history',
        controlType: 'Status/API',
        defaultValue: 'Recorded with completed Automation Runs',
        location: 'Changelog -> Action filter: Automation Runs -> Automation Period (expand) -> <channel> -> Quality Check (expand) -> Analyzed Streams -> Reason',
        locationTo: '/changelog',
        effect: 'Shows the final recovered, unavailable-after-recheck, or capacity-deferred reason retained for each analyzed stream.',
        useWhen: 'Use after Current Progress has cleared and you need the completed run evidence.',
        risk: 'The Changelog result belongs to its recorded run; verify the run time before treating it as current provider state.',
      },
    ],
    smokeChecks: [
      ...
      'A missing current bitrate either recovers during Bitrate Recheck, ends with Bitrate unavailable after recheck, or remains Capacity deferred when a provider/profile slot was full or capacity authority/usage could not be proved; inspect the reason before changing limits.',
  ...
  },
  {
    ...
    settings: [
      ...
      },
      {
        ...
        location: 'Teamarr Preflight -> Active Preflight Checks and Stream Checker -> Stream Checker Configuration -> Queue',
    ...
    smokeChecks: [
      ...
      'Queued event checks appear in Active Preflight Checks or Stream Checker queue context and run after the active channel, before lower-priority waiting work.',
  ...
  },
  {
    ...
    steps: [
      ...
      'Use the Shadow status API to confirm viewer output format when diagnosing a switch, because that backend-only field records whether the probe matched an fMP4 or MPEGTS viewer session.',
      'If Save reports that a newer Shadow configuration was loaded, review the reloaded fields and save again; the revision guard deliberately refuses to overwrite a concurrent edit.',
    ...
    settings: [
      ...
      },
      {
        ...
        location: 'GET /api/shadow-blank-monitor/status -> watched_channels[].viewer_output_format (backend-only)',
      ...
      },
      {
        ...
        effect: 'Continuously tracks viewers and performs periodic low-impact media probes while the real viewer remains connected. Legacy periodic config is normalized to continuous.',
        useWhen: 'Use as the normal viewer-protection behavior; there is no normal UI mode switch.',
        risk: 'Do not expect Scan Now or periodic polling to replace the continuous viewer-following loop.',
      },
      {
        name: 'Watcher API Key',
        controlType: 'Visible UI setting',
        defaultValue: 'Not configured',
        location: 'Shadow Monitor -> Configuration card -> Watcher API Key',
        effect: 'Stores the dedicated playback credential used by Shadow probes. The saved secret is never returned to the page; the UI reports only whether a key is configured.',
        useWhen: 'Configure a dedicated non-admin playback-user key reserved for Shadow before Start or Scan Now. Leave an already configured masked key unchanged unless it must be rotated.',
        risk: 'Never substitute an administrator or primary account key, and use a separate ordinary playback identity for real or synthetic real viewers. Clear Key explicitly removes the credential and prevents Start or Scan Now until another watcher key is saved.',
      },
      {
        name: 'Watcher User Agent',
        controlType: 'Visible UI setting',
        defaultValue: 'TiviMate preset with StreamFlow Shadow marker',
        location: 'Shadow Monitor -> Configuration card -> Watcher User Agent',
        effect: 'Sets the playback User-Agent used by Shadow probes while keeping a unique marker so StreamFlow can separate watcher clients from real viewers.',
        useWhen: 'Use the TiviMate default for normal live tests; choose another preset only when a provider behaves differently for a known player.',
        risk: 'A custom value should keep the StreamFlow Shadow marker; without it, Shadow may not reliably separate watcher probes from real clients.',
      },
      {
        name: 'Viewer Grace',
        controlType: 'Visible UI setting',
      ...
      },
      {
        ...
        useWhen: 'Enable for providers that can leave video moving while audio is broken, after validating the threshold on controlled streams.',
      ...
      },
      {
        ...
        useWhen: 'Enable for viewer protection when providers can deliver black-silent or video-only failure streams, after validating normal quiet programming.',
      ...
      },
      {
        ...
        useWhen: 'Learn provider-specific offline slates from a known bad stream, then explicitly enable Offline Image; stored references do not turn detection on by themselves.',
      ...
      },
      {
        ...
        useWhen: 'Enable for guarded live switching, especially for Silent Audio, no-audio, freeze, offline-image, and loop decisions.',
        risk: 'Turning it off can switch from one bad stream to another; loop-triggered live switches are blocked without it.',
      },
      {
        name: 'Configuration revision guard',
        controlType: 'Status/API',
        defaultValue: 'Applied by UI saves',
        location: 'Shadow Monitor -> Save -> "Configuration changed" notice',
        effect: 'Compares the loaded configuration revision during Save so a stale browser tab cannot silently overwrite another UI/API update or Start/Stop service action.',
        useWhen: 'If the conflict notice appears, review the automatically reloaded configuration and save the intended changes again.',
        risk: 'Do not repeatedly submit an old payload or bypass the conflict response; doing so can discard another operator or API client change.',
      },
      {
        name: 'Loop Detection',
        controlType: 'Visible UI setting',
      ...
      },
      {
        ...
        location: 'Shadow Monitor backend configuration and decision details',
        effect: 'Bounds how long Shadow compares sampled frame signatures before confirming repeated content.',
        useWhen: 'Use only for API-level troubleshooting of known loop streams; the normal UI keeps this as an internal Continuous guard.',
      ...
      },
      {
        ...
        defaultValue: '60 seconds',
        location: 'Shadow Monitor backend configuration and decision details',
        effect: 'Sets the bounded media-probe length and timing reference for watcher diagnostics.',
        useWhen: 'Use the backend default unless an API-level diagnostic needs a different probe window.',
      ...
      },
      {
        ...
        controlType: 'Status/API',
        defaultValue: '5 seconds',
        location: 'Shadow Monitor backend configuration and service loop',
        effect: 'Controls how often Continuous discovery checks active viewer sessions.',
        useWhen: 'Use the backend default for responsive watcher discovery.',
      ...
      },
      {
        ...
        location: 'Shadow Monitor -> Watched Now -> Waiting for healthy interval / Next probe',
        effect: 'Waits between clean continuous Dispatcharr proxy probes so Shadow does not act like a constant extra viewer when playback is healthy. Probe active, Last probe, and Next probe expose the current cadence.',
        useWhen: 'Keep the backend default for normal live monitoring. Lower only in controlled API-level diagnostics.',
      ...
      },
      {
        ...
        useWhen: 'Use the backend default so suspected faults confirm quickly after the first hit.',
      ...
      },
      {
        ...
        controlType: 'Status/API',
        defaultValue: '2 hits',
        location: 'Shadow Monitor backend configuration and decision history',
        effect: 'Requires repeated bad detections before switching.',
        useWhen: 'Use the backend default to reduce false positives from short video glitches.',
      ...
      },
      {
        ...
        location: 'Shadow Monitor backend configuration, cooldowns, and decision history',
        effect: 'Waits before another switch attempt on the same channel while watcher probes and fault events continue.',
        useWhen: 'Use as an internal flapping guard; inspect it when a bad channel is correctly detected but not switched.',
      ...
      },
      {
        ...
        location: 'Shadow Monitor backend configuration, switch summary, and decision history',
        effect: 'Limits successful stream switches for one channel in a rolling hour.',
        useWhen: 'Use as an internal flapping guard when a channel alternates between bad streams.',
    ...
    settings: [
      {
        ...
        location: 'Stream Checker -> Stream Checker Configuration -> Edit -> Stream Analysis tab -> Hardware Acceleration -> enable switch',
      ...
      },
      {
        ...
        controlType: 'Visible UI setting',
        defaultValue: 'auto',
        location: 'Stream Checker -> Stream Checker Configuration -> Edit -> Stream Analysis tab -> Hardware Acceleration -> Mode',
      ...
      },
      {
        ...
        location: 'Stream Checker -> Stream Checker Configuration -> Edit -> Stream Analysis tab -> Hardware Acceleration -> Device',
  ...
  },
  {
    ...
    settings: [
      ...
      },
      {
        ...
        location: 'Stream Checker -> Stream Checker Configuration -> Edit -> Stream Analysis tab -> FFmpeg Duration (seconds); Changelog -> Action filter: Automation Runs -> Automation Period (expand) -> <channel> -> Quality Check (expand) -> Visual Probe',
        locationTo: '/stream-checker',
      ...
      {
        name: 'Quality retry status',
        controlType: 'Status/API',
        defaultValue: 'Hidden unless a scheduled run is waiting to retry',
        location: 'Dashboard -> Automation Run -> Quality retry status; /api/automation/status -> run_status.scheduler_retry',
        locationTo: '/',
        effect: 'Shows the affected period, pending channel count, bounded retry attempt, next retry time, or exhausted state after a Connectivity Guard abort. Quality-stage retries use the current cache and do not restart provider refreshes.',
        useWhen: 'Use after a scheduled Quality Check stops because Dispatcharr was temporarily unreachable or too slow for a protected write.',
        risk: 'Do not start repeated manual Full Runs while a retry is scheduled; inspect the pending count and next retry first.',
      },
      {
        name: 'Needs Recheck bitrate status',
        controlType: 'Status/API',
        defaultValue: 'Shown only when media is playable but the current probe has no bitrate',
        location: 'Stream Checker -> Current Progress (active run) -> Stream Progress Tracking -> Status -> Needs Recheck',
        locationTo: '/stream-checker',
        effect: 'Keeps the stream playable and queues its lightweight serial bitrate recheck after the channel initial probes finish. If the recheck still has no bitrate, a previous measurement may be reused for ranking only; the current result remains N/A and visibly incomplete.',
        useWhen: 'Use when resolution, frame rate, codecs, and visual checks succeeded but FFmpeg did not report bitrate in this probe.',
        risk: 'Needs Recheck is not a dead-stream result and must not be treated as proof of zero bitrate.',
      },
      {
        name: 'Needs Recheck bitrate history',
        controlType: 'Status/API',
        defaultValue: 'Recorded with completed Automation Runs',
        location: 'Changelog -> Action filter: Automation Runs -> Automation Period (expand) -> <channel> -> Quality Check (expand) -> Analyzed Streams -> Reason',
        locationTo: '/changelog',
        effect: 'Preserves the completed run reason after the live Stream Progress Tracking table disappears.',
        useWhen: 'Use to distinguish a recovered bitrate from an unavailable or capacity-deferred recheck in a past run.',
        risk: 'A historical N/A result does not prove that a later run or provider refresh still lacks bitrate.',
      },
      {
      ...
      {
        name: 'Analytics history range',
        controlType: 'Visible UI setting',
        defaultValue: 'Last 7 Days',
        location: 'Analytics -> System Analytics header -> Date Range',
        locationTo: '/stats',
        effect: 'Offers the last 24 hours or the complete retained seven-day telemetry history. Changelog uses the same retained range.',
        useWhen: 'Use the shorter range for current incidents and seven days for run or provider trends.',
        risk: 'Telemetry older than seven days has already been pruned and is intentionally not offered as an empty range.',
      },
      {
