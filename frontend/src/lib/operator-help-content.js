...
export const operatorHelpDetailTopics = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Use Wait For Active Checks only when event checks should wait until automation and Stream Checker work are idle; leave it off to queue due events between other channel checks.',
    ...
    settings: [
      ...
      },
      {
        ...
        controlType: 'Visible UI setting',
        defaultValue: 'Off',
        location: 'Teamarr Preflight -> Configuration card -> Wait For Active Checks',
        effect: 'When on, due event checks wait while automation or Stream Checker work is active. When off, due events enter the priority queue and can run between other channel checks.',
        useWhen: 'Turn on only when event checks must never interleave with other checks; leave off for the default event-priority queue behavior.',
        risk: 'Turning it on can delay kickoff checks until regular automation or a long Stream Checker run has finished.',
        reference: {
          imageSrc: '/help/teamarr-preflight-busy-dark.jpg',
          triggerLabel: 'UI Screenshot',
          alt: 'Dark mode crop of the Teamarr Preflight Wait For Active Checks toggle.',
          caption: 'Wait For Active Checks lives in the Teamarr Preflight configuration card near the service enable switch and defaults off.',
          width: 239,
          height: 158,
        },
      },
