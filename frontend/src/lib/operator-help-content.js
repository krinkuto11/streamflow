...
export const operatorHelpDetailTopics = [
  ...
  },
  {
    ...
    visual: {
      ...
      steps: ['Read event', 'Main preflight -20 min', 'Auto-spaced pre-start retries', 'Auto-spaced post-start retries', 'Queue or complete'],
    },
    steps: [
      'Select the quality profile that represents event-channel check rules.',
      'Set Teamarr API Poll Interval to how often StreamFlow reads Teamarr managed-event state; 30-60 seconds keeps narrow windows reliable.',
      'Use the main preflight at 20 minutes before start for early, already-published event channels.',
      'Set Pre-Start Retries to the number of extra checks before start; StreamFlow distributes them between the main preflight and kickoff.',
      'Set Post-Start Retries to the number of checks after start; StreamFlow distributes them inside Post Start Grace.',
      'Keep Post Start Grace wide enough for post-start retries and the poll interval; the 5-minute default yields roughly +2-minute and +4-minute checks with two retries.',
    ...
    settings: [
      {
        ...
        effect: 'Controls how often StreamFlow reads Teamarr managed-event state and discovers due event buckets.',
        useWhen: 'Use 30-60 seconds for normal event automation; use longer intervals only when preflight windows are comfortably wide.',
        risk: 'A long poll interval can miss very narrow windows such as a 1-minute preflight offset or a short post-start grace.',
      },
      {
      ...
      },
      {
        ...
        controlType: 'Visible UI setting',
        defaultValue: '2 checks',
        location: 'Teamarr Preflight -> Configuration card -> Pre-Start Retries',
        effect: 'Adds this many extra pre-start checks, automatically distributed between the main preflight offset and event start.',
      ...
      {
        name: 'Post-Start Retries',
        controlType: 'Visible UI setting',
        defaultValue: '2 checks',
        location: 'Teamarr Preflight -> Configuration card -> Post-Start Retries',
        effect: 'Adds this many post-start checks, automatically distributed inside Post Start Grace.',
        ...
        reference: {
          ...
          triggerLabel: 'UI Screenshot',
          alt: 'Dark mode crop of Teamarr Preflight timing fields including API poll interval, preflight offset, retry counts, grace, and cooldown.',
          caption: 'Preflight Offset and Post Start Grace set timing windows; retry fields are counts that StreamFlow distributes automatically.',
          width: 492,
          height: 249,
        },
      },
      {
        ...
        useWhen: 'Keep it wide enough for the configured post-start retry count and the poll interval; 5 minutes with two retries yields roughly +2-minute and +4-minute checks.',
      ...
      },
      {
        ...
        controlType: 'Visible UI setting',
        defaultValue: 'On',
        location: 'Teamarr Preflight -> Configuration card -> Busy Handling',
      ...
      },
      {
        ...
        risk: 'It is not a preemption switch. If an event must be checked after kickoff, use Post-Start Retries rather than expecting priority to stop an active probe.',
      },
      {
    ...
    smokeChecks: [
      ...
      'A second post-start bucket can run around 4 minutes when the earlier post-start retry already ran.',
      'A post-start check is skipped as past if post-start grace is too narrow, so keep grace wider than the retry distribution and poll interval.',
